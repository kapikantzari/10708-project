...
class MultiGraphErgmExtraMotifs(AssociationModel):
    """
    ...
    def __init__(self, graphs, embeddings, assoc_mode=BILINEAR_MODE, reg=0.0,
                 dropout=0.0,
        ...
        """
        # input validation
        AssociationModel.__init__(self, graphs, embeddings, assoc_mode, dropout,
        ...
        self.transitivity_counts = self.add_cache_dict(
            'trans')  # keys are unordered relation triplets
        self.reciprocity_counts = self.add_cache_dict(
            'reci')  # keys are unordered relation triplets
        self.three_cycles_counts = self.add_cache_dict(
            '3cycle')  # keys are unordered relation triplets
        ...
        timeprint(
            'finished! computed {} features'.format(len(self.feature_vals)))
        ...
        if model_path is None and ergm_model_path is None:
            ...
            if not path_only_init:
                ...
                assert i == len(pc_list), \
                    '{} relation params read but length is {}'.format(i, len(
                        pc_list))
    ...
    def init_ergm_cache(self):
        ...
        for i, j, k in cyclic_triads(self.R):
            n1, n2, n3 = self.relation_names[i], self.relation_names[j], \
        ...
        for n, g in list(self.graphs.items()):
            ...
            self.transitivity_counts[(n1, n2, n2)] = osc['trans']
            self.reciprocity_counts[(n1, n2, n2)] = osc['reci']
            self.three_cycles_counts[(n1, n2, n2)] = osc['3cycle']
        ...
        for n1, n2 in combinations(self.relation_names, 2):
            ...
            self.transitivity_counts[(n1, n2, n2)] = tsc['trans']
            self.reciprocity_counts[(n1, n2, n2)] = tsc['reci']
            self.three_cycles_counts[(n1, n2, n2)] = tsc['3cycle']
        ...
        for n1, n2, n3 in combinations(self.relation_names, 3):
            ...
            self.transitivity_counts[(n1, n2, n2)] = ttsc['trans']
            self.reciprocity_counts[(n1, n2, n2)] = ttsc['reci']
            self.three_cycles_counts[(n1, n2, n2)] = ttsc['3cycle']
    ...
    def reread_cache(self, new_cache):
        ...
        self.transitivity_counts = self.cache['trans']
        self.reciprocity_counts = self.cache['reci']
        self.three_cycles_counts = self.cache['3cycle']
    ...
    def update_features(self):
        ...
        for (r1, r2, r3), val in list(self.transitivity_counts.items()):
            self.feature_vals[('trans', r1, r2, r3)] = val
        for (r1, r2, r3), val in list(self.reciprocity_counts.items()):
            self.feature_vals[('reci', r1, r2, r3)] = val
        for (r1, r2, r3), val in list(self.three_cycles_counts.items()):
            self.feature_vals[('3cycle', r1, r2, r3)] = val
    ...
    def ergm_score(self):
        ...
        f = dy.transpose(
            dy.inputVector([self.feature_vals[k] for k in self.feature_set]))
        return f * W
    ...
    def remove_edge(self, src_idx, trg_idx, rel, update_feats=True,
                    permanent=True,
        ...
        for r, g in list(self.graphs.items()):
            ...
            if g[trg_idx, src_idx] == 1:
                self.mutual_edge_counts[
                    find_key(self.mutual_edge_counts, (rel, r))] -= 1
            ...
            for r2, g2 in list(self.graphs.items()):
                ...
                if self.out_degs[r][0, src_idx] > 0 and self.in_degs[r2][
                    ...
                    self.transitive_closure_counts[
                        (r, r2, rel)] -= r_r2_betweens
        ...
        if not permanent and report_feat_diff:
            ...
            diff_keys = [k for k in self.feature_vals if
                         self.feature_vals[k] != cached_feats[k]]
            if len(diff_keys) > 0:
                print('\n'.join(['{}: from {} to {}' \
                                .format(k, cached_feats[k],
                                        self.feature_vals[k]) for k in
                                 diff_keys]))
    ...
    def add_edge(self, src_idx, trg_idx, rel, permanent=False, caches=None,
        ...
        if report_feat_diff:
            timeprint('changed feature values:')
            diff_keys = [k for k in self.feature_vals if
                         self.feature_vals[k] != backup_feats[k]]
            print('\n'.join(['{}: from {} to {}' \
                            .format(k, backup_feats[k], self.feature_vals[k])
                             for k in diff_keys]))
    ...
    def update_stars_cache_from_edge(self, rel, src, trg, added=True):
        ...
        curr_src_degs = {r: self.out_degs[r][0, src] for r in
                         self.relation_names}
        curr_trg_degs = {r: self.in_degs[r][0, trg] for r in
                         self.relation_names}
        ...
        if added:
            ...
            for r2 in self.relation_names:
                ...
                if curr_src_degs[rel] * curr_src_degs[r2] == 1:
                    self.out_two_star_counts[
                        find_key(self.out_two_star_counts, (rel, r2))] += 1
                if curr_src_degs[rel] == 1 and curr_src_degs[r2] >= 1:
                    self.out_two_p_star_counts[
                        find_key(self.out_two_p_star_counts, (rel, r2))] += 1
                if curr_src_degs[rel] == 2 and curr_src_degs[r2] == 1:
                    self.out_two_star_counts[
                        find_key(self.out_two_star_counts, (rel, r2))] -= 1
                    self.out_three_star_counts[
                        find_key(self.out_three_star_counts,
                                 (rel, rel, r2))] += 1
                    self.out_three_p_star_counts[
                        find_key(self.out_three_p_star_counts,
                                 (rel, rel, r2))] += 1
            ...
            for r2, r3 in combinations(self.relation_names, 2):
                ...
                if curr_src_degs[rel] * curr_src_degs[r2] * curr_src_degs[
                    r3] == 1:
                    self.out_three_star_counts[
                        find_key(self.out_three_star_counts,
                                 (rel, r2, r3))] += 1
                if curr_src_degs[rel] == 1 and curr_src_degs[r2] * \
                        curr_src_degs[r3] >= 1:
                    self.out_three_p_star_counts[
                        find_key(self.out_three_p_star_counts,
                                 (rel, r2, r3))] += 1
                if curr_src_degs[rel] == 2 and curr_src_degs[r2] * \
                        curr_src_degs[r3] == 1:
                    self.out_three_star_counts[
                        find_key(self.out_three_star_counts,
                                 (rel, r2, r3))] -= 1
            ...
            for r2 in self.relation_names:
                ...
                if curr_trg_degs[rel] * curr_trg_degs[r2] == 1:
                    self.in_two_star_counts[
                        find_key(self.in_two_star_counts, (rel, r2))] += 1
                if curr_trg_degs[rel] == 1 and curr_trg_degs[r2] >= 1:
                    self.in_two_p_star_counts[
                        find_key(self.in_two_p_star_counts, (rel, r2))] += 1
                if curr_trg_degs[rel] == 2 and curr_trg_degs[r2] == 1:
                    self.in_two_star_counts[
                        find_key(self.in_two_star_counts, (rel, r2))] -= 1
                    self.in_three_star_counts[
                        find_key(self.in_three_star_counts,
                                 (rel, rel, r2))] += 1
                    self.in_three_p_star_counts[
                        find_key(self.in_three_p_star_counts,
                                 (rel, rel, r2))] += 1
                if curr_trg_degs[rel] == 1 and curr_trg_degs[r2] == 2:
                    self.in_three_star_counts[
                        find_key(self.in_three_star_counts, (rel, r2, r2))] += 1
                if curr_trg_degs[rel] == 1 and curr_trg_degs[r2] >= 2:
                    self.in_three_p_star_counts[
                        find_key(self.in_three_p_star_counts,
                                 (rel, r2, r2))] += 1
            ...
            for r2, r3 in combinations(self.relation_names, 2):
                if r2 == rel or r3 == rel: continue
                if curr_trg_degs[rel] * curr_trg_degs[r2] * curr_trg_degs[
                    r3] == 1:
                    self.in_three_star_counts[
                        find_key(self.in_three_star_counts, (rel, r2, r3))] += 1
                if curr_trg_degs[rel] == 1 and curr_trg_degs[r2] * \
                        curr_trg_degs[r3] >= 1:
                    self.in_three_p_star_counts[
                        find_key(self.in_three_p_star_counts,
                                 (rel, r2, r3))] += 1
                if curr_trg_degs[rel] == 2 and curr_trg_degs[r2] * \
                        curr_trg_degs[r3] == 1:
                    self.in_three_star_counts[
                        find_key(self.in_three_star_counts, (rel, r2, r3))] -= 1
            ...
            for r2, r3 in combinations(self.relation_names, 2):
                if r2 == rel or r3 == rel: continue
                if curr_src_degs[rel] == 0 and curr_src_degs[r2] * \
                        curr_src_degs[r3] == 1:
                    self.out_three_star_counts[
                        find_key(self.out_three_star_counts,
                                 (rel, r2, r3))] -= 1
            ...
            for r2, r3 in combinations(self.relation_names, 2):
                if r2 == rel or r3 == rel: continue
                if curr_trg_degs[rel] == 0 and curr_trg_degs[r2] * \
                        curr_trg_degs[r3] == 1:
                    self.in_three_star_counts[
                        find_key(self.in_three_star_counts, (rel, r2, r3))] -= 1
                if curr_trg_degs[rel] == 0 and curr_trg_degs[r2] * \
    ...
    def update_stars_cache(self, rel):
        ...
        for r2 in self.relation_names:
            if r2 == rel: continue
            tsc = two_rel_star_counts(in_ds, out_ds, self.in_degs[r2],
                                      self.out_degs[r2])
            self.in_two_star_counts[
                find_key(self.in_two_star_counts, (rel, r2))] = tsc['i2sc']
            self.out_two_star_counts[
                find_key(self.out_two_star_counts, (rel, r2))] = tsc['o2sc']
            self.in_three_star_counts[
                find_key(self.in_three_star_counts, (rel, rel, r2))] = tsc[
                'i3sc112']
            self.out_three_star_counts[
                find_key(self.out_three_star_counts, (rel, rel, r2))] = tsc[
                'o3sc112']
            self.in_three_star_counts[
                find_key(self.in_three_star_counts, (rel, r2, r2))] = tsc[
                'i3sc122']
            self.out_three_star_counts[
                find_key(self.out_three_star_counts, (rel, r2, r2))] = tsc[
                'o3sc122']
            self.in_two_p_star_counts[
                find_key(self.in_two_p_star_counts, (rel, r2))] = tsc['i2psc']
            self.out_two_p_star_counts[
                find_key(self.out_two_p_star_counts, (rel, r2))] = tsc['o2psc']
            self.in_three_p_star_counts[
                find_key(self.in_three_p_star_counts, (rel, rel, r2))] = tsc[
                'i3psc112']
            self.out_three_p_star_counts[
                find_key(self.out_three_p_star_counts, (rel, rel, r2))] = tsc[
                'o3psc112']
            self.in_three_p_star_counts[
                find_key(self.in_three_p_star_counts, (rel, r2, r2))] = tsc[
                'i3psc122']
            self.out_three_p_star_counts[
                find_key(self.out_three_p_star_counts, (rel, r2, r2))] = tsc[
                'o3psc122']
        ...
        for r2, r3 in combinations(self.relation_names, 2):
            ...
            self.in_three_star_counts[
                find_key(self.in_three_star_counts, (rel, r2, r3))] = ttsc[
                'i3sc']
            self.out_three_star_counts[
                find_key(self.out_three_star_counts, (rel, r2, r3))] = ttsc[
                'o3sc']
            self.in_three_p_star_counts[
                find_key(self.in_three_p_star_counts, (rel, r2, r3))] = ttsc[
                'i3psc']
            self.out_three_p_star_counts[
                find_key(self.out_three_p_star_counts, (rel, r2, r3))] = ttsc[
                'o3psc']
    ...
    def save(self, filename, initial_weights=None, save_with_embeddings=True):
        ...
        dy.save(filename + '.dyn', [self.ergm_weights] + \
                [self.word_assoc_weights[r] for r in
                 self.relation_names])  # order matters for loading
    ...
    def save_weights(self, filename, initial_weights):
        ...
        with open(filename + '.scores', 'w') as file_out:
            file_out.write(
                'feat cat\trelations\tfinal weight\tinitial weight\tdiff\n')
